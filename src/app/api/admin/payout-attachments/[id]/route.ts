...
// DELETE /api/admin/payout-attachments/:id  — owner removes one evidence file.
export async function DELETE(
  ...
  try {
    ...
    const existing = await pool.query<{ cloudinary_public_id: string | null }>(
      `SELECT cloudinary_public_id FROM partner_payout_attachments WHERE id = $1`,
      [id]
    );
    ...
    if (existing.rows[0].cloudinary_public_id) {
      try {
        ...
        console.warn(
          "[admin/payout-attachments DELETE] cloudinary cleanup failed:",
          cloudErr instanceof Error ? cloudErr.message : cloudErr
        );
      }
    }
    await pool.query(`DELETE FROM partner_payout_attachments WHERE id = $1`, [id]);
    return NextResponse.json({ success: true });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : "Failed to delete attachment";
    console.error("[admin/payout-attachments DELETE] error:", err);
